...
export const createCohort = async (req, res) => {
  try {
    const { name, description, year, tags, startDate, endDate, maxParticipants } = req.body;
...
export const getCohortById = async (req, res) => {
  try {
    ...
    if (req.user?.role !== 'ADMIN') {
      const membership = await UserCohort.isUserInCohort(req.user._id, cohort._id);
...
export const updateCohort = async (req, res) => {
  try {
    ...
    if (req.user.role !== 'ADMIN' && cohort.createdBy.toString() !== req.user._id.toString()) {
      return forbiddenResponse(res, 'Access denied');
    }

    const { name, description, year, tags, startDate, endDate, status, maxParticipants } = req.body;
    ...
    if (startDate !== undefined) cohort.startDate = startDate ? new Date(startDate) : null;
    if (endDate !== undefined) cohort.endDate = endDate ? new Date(endDate) : null;
...
export const getCohortMembers = async (req, res) => {
  try {
    ...
    return successResponse(res, result, 'Cohort members retrieved successfully');
  } catch (error) {
...
export const addMemberToCohort = async (req, res) => {
  try {
    ...
    const existingMembership = await UserCohort.isUserInCohort(userId, cohort._id);
    if (existingMembership) {
      return errorResponse(res, new Error('User is already a member of this cohort'));
    }
    ...
    return successResponse(res, userCohort, 'User added to cohort successfully', 201);
  } catch (error) {
...
export const updateMemberRole = async (req, res) => {
  try {
    ...
    const membership = await UserCohort.findOne({ userId, cohortId: cohort._id });
